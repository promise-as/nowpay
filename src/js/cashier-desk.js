$(function () {
  ...
  var min, sec
  //读取cookie
  if ($.cookie("countDownEndTime") !== undefined
    && !isNaN($.cookie("countDownEndTime"))) {  //读取到了cookie值
  ...
  //5.用到期时间减去当前时间获取倒计时
  var countDownEndTime = $.cookie("countDownEndTime");
  if (countDownEndTime === null || countDownEndTime === undefined
    || countDownEndTime === 'undefined' || countDownEndTime === 'null') {
    var now = new Date().getTime()  //当前时间戳
